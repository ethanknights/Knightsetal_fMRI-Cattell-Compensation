...
m<-polr(ordy ~ age, data=df_subset, Hess=TRUE)
summary(m)
...
#Get bayes factors
#For absence of RH boost (i.e. we see RH reduction)
library(BayesFactor)
generalTestBF(ordy ~ age, data = df_subset)
...
ggplot(df_subset, aes(age, fill = fct_rev(ordy))) +
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1.5),
        ...
        text = element_text(size=24)) +
  #Set Colours
  scale_fill_manual( values = c("springgreen","grey","Red")) +
  #Remove space around axis
  coord_cartesian(xlim = c(19,87) , ylim = c(0,1), expand = FALSE)
ggsave(file.path(outImageDir,'Boost_geom_density.png'),
       width = 25, height = 25, units = 'cm', dpi = 300)
