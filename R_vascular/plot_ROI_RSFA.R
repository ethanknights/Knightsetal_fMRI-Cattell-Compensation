...
dev.copy(device = tiff, filename = file.path(outImageDir,'RSFA_cuneal_interaction_plot.tiff'), width = pxwidth, height = pxheight); dev.off()
...
plot(ggplot_gtable(qq))
grid.draw(plot(ggplot_gtable(qq)))
dev.copy(device = tiff, filename = file.path(outImageDir,'RSFA_frontal_interaction_plot.tiff'), width = pxwidth, height = pxheight); dev.off()
